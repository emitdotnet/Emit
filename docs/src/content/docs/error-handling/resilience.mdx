---
title: Resilience
description: Circuit breakers and rate limiting for Kafka consumer groups.
---

Beyond retry policies, Emit provides two resilience mechanisms at the consumer group level: circuit breakers to stop hammering a downstream system that is clearly broken, and rate limiting to protect a system that is struggling.

## Circuit breaker

A circuit breaker tracks failures in a sliding window. When failures exceed the threshold, it opens, pausing the consumer group entirely until the pause duration elapses.
