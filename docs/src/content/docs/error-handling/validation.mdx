...
A null payload is treated as a deterministic validation failure by default. To let null bypass FluentValidation (log-compaction tombstones, for example), pass `allowNullMessages: true`. See [Null payloads](#null-payloads) for the full story.
...
When a message is dead-lettered due to validation failure, the `x-emit-exception-type` header contains `Emit.MessageValidationException` and the individual error messages appear in `x-emit-exception-message`.
...
A null payload at the consumer means different things in different contexts. On a normal topic it's almost always a defect: a producer bug or a serialization regression. On a log-compacted topic it's a tombstone signaling a key was deleted. Emit doesn't guess which case applies. By default, null is treated as a validation failure; you opt into the tombstone interpretation explicitly.

The default routes a null payload through the configured validation error action (dead-letter or discard) without invoking the handler.

To accept nulls and let your handler observe them, pass `allowNullMessages: true`:

```csharp
group.ValidateWithFluentValidation(
    onFailure => onFailure.DeadLetter(),
    allowNullMessages: true);
```

FluentValidation is bypassed for null payloads only. Non-null messages still flow through the configured `IValidator<T>` rules. Your handler then receives the null and decides what to do with it (typically treating it as a delete signal).

If you'd rather drop null payloads silently, with no handler invocation and no dead-letter entry, use `.SkipNullPayloads()` upstream of validation:

```csharp
group
    .SkipNullPayloads()
    .ValidateWithFluentValidation(onFailure => onFailure.DeadLetter());
```

`SkipNullPayloads()` is a small wrapper over `.Filter(...)` constrained to reference-typed messages. Filtered items short-circuit before validation runs; offsets advance and no exception or DLQ entry is produced.

When to pick which: use `allowNullMessages: true` when the handler needs to see the null (cache invalidation, delete propagation). Use `.SkipNullPayloads()` when nulls are noise and the handler should never see them.
