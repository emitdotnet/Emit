...
| `emit.pipeline.consume.messages` | Counter | `provider`, `result`, `consumer` | Individual messages processed (batch-aware: counts each item in a batch) |
...
**Prerequisite:** these instruments are only populated when `KafkaClientOptions.StatisticsInterval` is set to a non-zero value (e.g. `TimeSpan.FromSeconds(5)`). Without it, the callback never fires and all broker meter values remain zero.
