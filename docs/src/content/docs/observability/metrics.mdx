...
| `emit.consumer.rate_limit.acquired` | Counter | — | Rate limit permits acquired |
| `emit.consumer.rate_limit.wait_duration` | Histogram (s) | — | Time waiting for a rate limit permit |
| `emit.consumer.dlq.produced` | Counter | `reason`, `dlq_topic` | Messages forwarded to DLQ |
| `emit.consumer.dlq.produce_errors` | Counter | `reason`, `dlq_topic` | Failures while forwarding to DLQ |
...
| Instrument | Type | Tags | Description |
|---|---|---|---|
| `emit.outbox.enqueued` | Counter | `provider` | Entries written to the outbox |
| `emit.outbox.processing.duration` | Histogram (s) | `provider`, `result` | Time to deliver one entry |
| `emit.outbox.processing.completed` | Counter | `provider`, `result` | Entries processed |
| `emit.outbox.critical_time` | Histogram (s) | `provider` | End-to-end latency from enqueue to delivery |
...
| `emit.outbox.worker.active_groups` | Gauge | — | Concurrent group processing count |
...
| `emit.kafka.consume.dlq.produced` | Counter | `consumer_group`, `source_topic`, `dlq_topic` | Deserialization errors forwarded to DLQ |
