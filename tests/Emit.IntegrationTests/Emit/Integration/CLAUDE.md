...
| `TransactionProbe.cs` | Records the ambient transaction observed inside a handler, plus `IAmbientSessionInspector` for provider session handles | Assert that a handler ran inside (or outside) a transaction |
