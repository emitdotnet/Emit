...
/// the middleware implements <see cref="IMiddleware{TContext}"/> for <see cref="OutboundContext{T}"/>.
/// </summary>
...
public interface IOutboundConfigurable<TMessage> : IOutboundPipelineConfigurable
{
    ...
    /// for <see cref="OutboundContext{T}"/> with the builder's message type.
    /// </typeparam>
    ...
    IOutboundConfigurable<TMessage> Use<TMiddleware>(MiddlewareLifetime lifetime = default)
        where TMiddleware : class, IMiddleware<OutboundContext<TMessage>>;
}
